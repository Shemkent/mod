...
## Overview
War goals define what a war is actually fought for and what can be demanded in the peace deal. Each war goal has a hardcoded `type` that determines the broad category of peace options available (province seizure, total subjugation, humiliation, etc.), then overlays script-controlled costs, restrictions, and call-in rules on top. War goals are referenced by casus belli definitions via `war_goal_type`.

Peace treaty scripts (in `peace_treaties/`) extend the system with scripted demands beyond automatic land claims — forcing conversions, stealing maps, humiliating rulers, seizing specific provinces by script, and more. These demands stack on top of the war goal's peace deal template.
...
<treaty_id> = {
    ...
    ai_desire = { <script value> }  # AI weighting for accepting/demanding this treaty (loser and winner both read it)
...
| `ticking_war_score` | Monthly warscore shift (engine default 1; most vanilla war goals use 0.5) | 0 disables ticking; higher values make wars resolve faster |
...
| `war_name_is_country_order_agnostic` | Makes the war name the same regardless of which side declared | Without this, ENG v FRA and FRA v ENG generate different war-name lookups |
| `call_in_overlord` | Whether the side's overlord is automatically joined | Behaviour is type-dependent; verify for each hardcoded type |
| `call_in_subjects` | Whether the side's subjects are automatically joined | Independent of `call_in_overlord`; both can be set per side |
...
- **`ticking_war_score`** defaults to 1 per the engine, but most vanilla war goals explicitly set 0.5. Setting it to `0` makes warscore purely battle-driven.
- **`call_in_subjects`** and **`call_in_overlord`** are independent per-side flags. Setting one does not affect the other. Both can be set on both attacker and defender blocks.
- **`select_trigger` in peace treaties** is a multi-stage targeting system with many sub-fields (source narrowing, AI overrides, map mode, pre-evaluation sorting). Refer to `peace_treaties/readme.txt` for the full schema — it is extensive and not replicated here.
...
conquer_province = {
    ...
    defender = {
        # empty block: defender uses default peace deal options for take_province type
        # (whether the defender can take land depends on the hardcoded type, not this block)
    }
